...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    ...
    if 'eagle' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            model.text_encoder = BertModel.from_pretrained('google-bert/bert-base-uncased',torch_dtype=model.dtype).to(device = device)
        else:
            ...
            if model.get_vision_tower().num_samples < model.get_vision_tower().num_towers:
                print("create_text_encoder..")
                model.text_encoder = BertModel.from_pretrained('google-bert/bert-base-uncased',torch_dtype=model.dtype).to(device = device)
            # pdb.set_trace()
    else:
        # Load language model
    ...
    if 'eagle' or 'sample' in model_name.lower():
