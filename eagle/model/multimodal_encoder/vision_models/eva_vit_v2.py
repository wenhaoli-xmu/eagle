...
class Block(nn.Module):
    ...
    def _forward(self, x):
        ...
        x, windows_attention = self.attn(x, output_attentions = True)

        # Reverse window partition
        if self.window_size > 0:
            x = window_unpartition(x, self.window_size, pad_hw, (H, W))
            global_attention = window_unpartition(windows_attention, self.window_size, pad_hw, (H, W))
    ...
    def forward(self, x, with_cp=False):
        ...
        if with_cp:
            x, attn = cp.checkpoint(self._forward, x)
        else:
            x, attn = self._forward(x)
        return x, attn
...
class EVAViT(nn.Module):
    ...
    def forward(self, x):
        ...
        for blk in self.blocks:
            x, attn = blk(x, with_cp=self.gradient_checkpointing)   # b, h, w, c
        attns.append(attn)
...
class EVAVITVisionTower(nn.Module):
    ...
    def get_input_embedding(self):
        return self.vision_tower.patch_embed.proj
    ...
    def forward(self, images):
        if type(images) is list:
            ...
            image_forward_out, attns = self.vision_tower(images.to(device=self.device, dtype=self.dtype))

        return image_forward_out, attns
