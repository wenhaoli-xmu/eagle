...
class SAMVisionTower(nn.Module):
    ...
    def forward(self, images):
        if type(images) is list:
            ...
            return_dict = self.vision_tower(images.to(device=self.device), output_attentions = True)
            image_feature = return_dict.last_hidden_state.flatten(start_dim=1, end_dim=2).to(device=self.device)
            attn = return_dict.attentions
