...
class EagleMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            self.sampler = getattr(config, 'mm_vision_sample_type', "uniform")
            if 'unpad' in getattr(config, 'mm_patch_merge_type', ''):
                self.image_newline = nn.Parameter(
                    torch.empty(config.hidden_size, dtype=self.dtype)
                )
            if getattr(config, 'mm_patch_merge_type', False):
                self.build_gate_fc()
    
    def build_gate_fc(self): # [B, N**2, C]  --> [B, N, N, C] -->[B,C] --> [B,1]
        
        # encoder_hidden_size = 1024 * self.vision_tower.num_towers
        # hidden_size = 1024
        # depth = 3
        # RegBlock = partial(
        #     RegStage,
        ...
        #     hidden_size,
        # )
        # sampler1 = nn.AdaptiveAvgPool2d((32, 32))
        # s2 = RegBlock(
        #     depth,
        #     hidden_size,
        #     hidden_size,
        # )
        # sampler2 = nn.AdaptiveAvgPool2d((1, 1))
        # self.gate_fc = nn.Sequential(s1, sampler1, s2, sampler2)
        pass
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        # if self.config.mm_vision_sample_feature:
        #     self.build_gate_fc()
...
class EagleMetaForCausalLM(ABC):
    ...
    def encode_images(self, images, text_features=None):
        ...
        #split_image_featurs = torch.split(image_features, self.get_vision_tower().num_towers, dim=1)
        
        # self.get_mask(split_image_featurs, text_features, image_features.device, image_features.dtype)
        image_features = self.get_model().mm_projector(image_features)
        # image_features = self.get_model().mm_projector(image_features, self.get_vision_tower().sampled_indices)
        
        # for i, module in enumerate(self.get_model().mm_projector):
        #     if i == 0:
        #         if hasattr(self.get_vision_tower(), "samples"):
        #             image_features = module(image_features, self.get_vision_tower().sampled_indices)
        #         else:
